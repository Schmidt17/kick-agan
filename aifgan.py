...
def discriminator(seq_len):
	...
	model = tf.keras.Sequential([
			tf.keras.layers.Conv1D(filters=64, kernel_size=5, 
				strides=2, padding='same', activation='relu', input_shape=(seq_len, 1)),
			# shape=(None, seq_len*2, 64)
			tf.keras.layers.Dropout(0.3),
			tf.keras.layers.Conv1D(filters=128, kernel_size=5, 
				strides=2, padding='same', activation='relu'),
			# shape=(None, seq_len*4, 128)
			tf.keras.layers.Dropout(0.3),
...
def generator(seq_len):
	...
	model = tf.keras.Sequential([
			...
			tf.keras.layers.Conv2DTranspose(filters=128, kernel_size=(1, 5),  use_bias=False,
				strides=(1, 1), padding='same', activation='relu'),
			# shape=(None, 1, seq_len/4, 128)

			tf.keras.layers.Conv2DTranspose(filters=64, kernel_size=(1, 5),  use_bias=False,
				strides=(1, 1), padding='same', activation='linear'),
			tf.keras.layers.BatchNormalization(),
			tf.keras.layers.LeakyReLU(),
			# shape=(None, 1, seq_len/4, 64)

			tf.keras.layers.Conv2DTranspose(filters=32, kernel_size=(1, 5),  use_bias=False,
				strides=(1, 2), padding='same', activation='relu'),
			# shape=(None, 1, seq_len/2, 32)

			tf.keras.layers.Conv2DTranspose(filters=1, kernel_size=(1, 5),  use_bias=False,
				strides=(1, 2), padding='same', activation='tanh'),
...
audio_data = np.array([np.pad(data, (0, max_len-data.shape[0]), mode='edge') for data in audio_data])
...
def train_step(train_audio):
	...
	disc_opt.apply_gradients(zip(disc_gradients, D.trainable_variables))
...
gen = G(seed, training=False)
AudioLoader.save_aif((gen[0, :, 0].numpy()-0.5) * 2**15, "test.aif", downs)
